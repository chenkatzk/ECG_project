...
def parse_args():
    ...
    parser.add_argument('--phase', type=str, default='test', help='test or train')
...
def apply_thresholds(test_loader, net, device, thresholds,train_test_flag):
    output_list, label_list = [], []
    ...
    plot_cm(y_trues, y_preds,train_test_flag)


def plot_cm(y_trues, y_preds,train_test_flag, normalize=True, cmap=plt.cm.Blues):
    classes = ['Mls', 'TAb', 'LVH', 'abQRS', 'LAD', 'MI', 'SNR']
    for i, label in enumerate(classes):
        ...
        if train_test_flag==1:
            plt.savefig(f'results/{label}_TEST.png')
        if train_test_flag ==0:
            plt.savefig(f'results/{label}_VALIDATION.png')
        plt.close(fig)
...
#this is for the train:
    if args.phase != 'test':
        ...
        apply_thresholds(val_loader, net, device, thresholds,0)
    
#this is for test:
    print('Results on test data:')
    apply_thresholds(test_loader, net, device, thresholds,1)
